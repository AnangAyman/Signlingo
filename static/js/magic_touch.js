...
function scheduleSpawn() {
    ...
    spawnInterval = setTimeout(() => {
        ...
        // Slower ramp-up for multiple enemies
        let enemiesToSpawn = 1;
        if (waveCount > 8) enemiesToSpawn = Math.floor(Math.random() * 2) + 1;  // 1 to 2 enemies
        if (waveCount > 20) enemiesToSpawn = Math.floor(Math.random() * 2) + 2; // 2 to 3 enemies
        ...
        enemySpeed = Math.min(2.0, enemySpeed + 0.02);
...
function spawnEnemy(startX) {
    ...
    // Use the safe X coordinate generated by the anti-overlap logic
